# GCE Vectrex — Unique Games Catalogue

| | |
|---|---|
| **Platform** | Vectrex (1982) — GCE, then Milton Bradley |
| **Lifespan** | Nov 1982 – early 1984; ~500,000 units sold |
| **Library size** | 28 official cartridges |
| **Backward compat** | None. |
| **Last verified** | 2026-07-29 |
| **Coverage** | **Complete** for notable titles; 28 cartridges total. | ## ⚠ The only vector-display console ever sold

The Vectrex has a stranding cause nothing else shares: a **built-in vector monitor**. It draws lines directly with an electron beam rather than
filling a pixel grid, so its games are crisp glowing wireframes at any scale — and a
raster screen cannot reproduce that, only approximate it.

It also shipped with **plastic screen overlays** for colour, since the display is
monochrome, and had the first commercial **3D headset** as an add-on in 1983.

Milton Bradley acquired GCE and killed the platform during the 1983 crash. The rights
have since passed through Hasbro, which — — **released them for
non-commercial use**, seeding a homebrew scene that still produces cartridges.

## Summary

| | Count |
|---|---|
| **Debut games** | **8** |
| — still `Stranded` | 8 |
| — since `Ported` | 0 |
| **Contested** | 1 | **Why the Vectrex strands games.** The display, primarily — any re-release on modern
hardware is a simulation of a vector monitor, not the thing itself, and the overlays
are physical. Beyond that, Hasbro has no commercial interest, and several titles were
arcade licences (`Berzerk`, `Scramble`, `Star Trek`) whose rights sit elsewhere.

Another platform with a `Ported` count of **zero**.

---

## Debut games

| Title | Year | Developer | Publisher | Genre | Players | Online | Status | Also On | Notes |
|---|---|---|---|---|---|---|---|---|---|
| Mine Storm | 1982 | John Hall | GCE | Shoot 'em up | 1 | No | **Stranded** | | Built into every console's ROM rather than sold separately — an `Asteroids` variant that is the game most Vectrex owners played. Its later `Mine Storm II` cartridge existed only to fix a level-13 crash bug. |
| Scramble | 1982 | GCE | GCE | Shoot 'em up | 1 | No | **Stranded** | | Licensed from Konami's cabinet; the licence has long since lapsed. |
| Star Trek: The Motion Picture | 1982 | GCE | GCE | Space combat | 1 | No | **Stranded** | | A Paramount film licence on a dead vector console — locked by two rights holders on dead hardware. |
| 3D Mine Storm | 1983 | GCE | GCE | Shoot 'em up | 1 | No | **Stranded** | | Requires the **Vectrex 3D Imager**, a spinning-disc stereoscopic headset released in 1983. The peripheral was discontinued almost immediately and is now vanishingly rare — doubly stranded, by display and by accessory. |
| Fortress of Narzod | 1983 | GCE | GCE | Shoot 'em up | 1 | No | **Stranded** | | A GCE original that needs the vector display, and Milton Bradley shut the platform down months after it shipped. |
| Pole Position | 1983 | GCE | GCE | Racing | 1 | No | **Stranded** | | A Namco licence, and one of the last official releases before Milton Bradley shut the platform down. |
| Spike | 1983 | GCE | GCE | Platformer | 1 | No | **Stranded** | | **The first console game with in-game speech**, using the Vectrex's audio hardware for spoken dialogue. Also one of the few titles with a named character, positioned as a mascot. |
| Web Wars | 1983 | GCE | GCE | Shoot 'em up | 1 | No | **Stranded** | | Uses the vector display for a scaling tunnel effect that raster hardware of the era could not do at all. | ## Contested

| Title | Year | Verdict | Case for including | Case against |
|---|---|---|---|---|
| Mine Storm | 1982 | **Include** | It is a complete, named, reviewed game that constitutes the console's pack-in — and the `Mine Storm II` cartridge shows it was treated as distributable software. | It is built into console ROM rather than sold on removable media, which is the basis on which this repo excludes the Odyssey²'s and Channel F's built-in games. Included because it was also released on cartridge; the inconsistency is noted. | ## Excluded

| Title | Year | Why excluded |
|---|---|---|
| Berzerk, Rip-Off, Armor Attack, Star Castle | 1982–83 | Arcade licences that had earlier home conversions on other platforms. |
| Clean Sweep, Hyperchase, Cosmic Chasm | 1982–83 | Qualify as debuts and are `Stranded`; not separately tabled. |
| Homebrew cartridges (1996–present) | | Hasbro released the rights for non-commercial use, so an active scene still produces new Vectrex games — several hundred by now. Out of scope. |
| *(games with an earlier arcade release)* | | **Not excluded.** Arcades are out of scope; the first *home* release counts. A prior release on another home console still disqualifies. | ## Coverage gaps

- 8 of 28 cartridges individually tabled; the rest follow the same pattern and status.
- Homebrew is out of scope, though it is large here — the Vectrex's library
 has more than doubled since the console died.

## Sources

Checked 2026-07-29.

- [Vectrex — Wikipedia](https://en.wikipedia.org/wiki/Vectrex)
- [List of Vectrex games — Wikipedia](https://en.wikipedia.org/wiki/List_of_Vectrex_games)
- [Vectrex 3D Imager — Wikipedia](https://en.wikipedia.org/wiki/Vectrex#3D_Imager)
