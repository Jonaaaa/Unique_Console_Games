...
[`CONSISTENCY.md`](CONSISTENCY.md) lists the 20 checks that enforce it.
...
| `Genre` | Short and consistent; reuse terms already in the file. Qualify with a slash or a word, `Puzzle / stealth` and `Rhythm roguelike`, never a parenthesis. **A qualifier that carries meaning is not drift**: `Action / city sim` and `Online RPG` say something the bare genre does not, and 105 genres are used once for that reason. A spelling variant is drift: `Stealth` beside eleven `Stealth action` is the same genre twice. |
...
| `Language` | The language the game shipped in, where it is known and worth stating. Leave empty rather than assuming; an empty cell means unrecorded, not English. **On hardware that never left one market, every row carries it**, because on those platforms it is known for all of them: the WonderSwan, PC-FX, Loopy, Playdia and the three early Japanese consoles are all `Japanese`, and `Geraldinho` on a Brazilian Master System is `Portuguese`. A game released in Japan *and* PAL shipped in English somewhere, so it stays empty. |
